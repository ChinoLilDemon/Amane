...
export default new class extends Command{
    ...
    exec(msg: discord.Message, cmd: string, args: string[], prefix: string, language: string): void {
        ...
        Promise.all([
            ...
            .then(v=>{
                embed.setTitle('INFO');
                embed.addField(dialogs.get(language).info.author, 'ChinoLilDemon#0001', true);
                embed.addField(dialogs.get(language).info.fw, 'discord.js & node-canvas', true);
                embed.addField(dialogs.get(language).info.invite, `[click here](${v[0]})`);
                embed.addField(dialogs.get(language).info.advert, `[click here](https://discord.gg/BqMJJEw)`);
